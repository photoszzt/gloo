...
Context::Context(std::shared_ptr<Device> device, int rank, int size, std::string shmem_path)
...
std::unique_ptr<transport::Pair>& Context::createPair(int rank) {
  pairs_[rank] = std::unique_ptr<transport::Pair>(
      new tcp_shmem::Pair(this, device_.get(), rank, getTimeout()));
  return pairs_[rank];
}
...
std::unique_ptr<transport::UnboundBuffer> Context::createUnboundBuffer(
    void* ptr,
    size_t size) {
  auto buf = new tcp_shmem::UnboundBuffer(shared_from_this(), ptr, size);
...
void Context::signalException(const std::string& msg) {
  ...
  for (auto& pair : pairs_) {
    if (pair) {
      reinterpret_cast<tcp_shmem::Pair*>(pair.get())->signalExceptionExternal(msg);
    }
  }
}
