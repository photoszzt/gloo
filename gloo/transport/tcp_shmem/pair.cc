...
#include "gloo/transport/tcp_shmem/buffer.h"
#include "gloo/transport/tcp_shmem/context.h"
#include "gloo/transport/tcp_shmem/unbound_buffer.h"
...
void Pair::listen() {
  ...
  self_ = tcp::Address::fromSockName(fd);
...
void Pair::connect(const tcp::Address& peer) {
  std::unique_lock<std::mutex> lock(m_);
  int rv;
  socklen_t addrlen;
  throwIfException();
...
void Pair::handleConnected() {
  ...
  // Reset addresses
  self_ = tcp::Address::fromSockName(fd_);
  peer_ = tcp::Address::fromPeerName(fd_);
...
void Pair::send(
    ...
    size_t offset,
    size_t nbytes) {
  auto buf = static_cast<tcp_shmem::UnboundBuffer*>(tbuf)->getWeakNonOwningPtr();
